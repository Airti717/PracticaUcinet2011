...
Al ver que todo nos cuadra, decidimos hacer el ACP de dimensión 6. Aún así, esta reducción no nos da un trabajo fácil y cómodo a la hora de hacer las impresiones de los datos.
...
```{r}
k_medoids = pam(x = head(data_cluster,50),k = 4,metric = "euclidean",stand = FALSE)
...
clara = clara( x = head(data_cluster,100), 4, metric = "euclidean", stand = FALSE, samples = 5, pamLike = FALSE)
table(head(data_cluster,100)$ICUType, clara$cluster)



# Si probamos a usar las distancias gower que hemos calculado arriba, aunque no sabemos printarlo en gráfica de puntos
clusters <- pam(x = as.data.frame(distancia), k = 4, diss = TRUE)
